...
当前状态：系统审查中的确定性错误已升级到任务契约v2并通过工程回归；旧15例mixed用原检查点在修正行程后15/15真实退出且成功，无新训练。下一步先做修正版完整基线与重复性校对，不直接续旧训练协议。姿态目标、实际扭矩硬限及纵向错相课程尚未改动，详见`wheelleg_warp/results/system_fixes_20260922/REPORT.md`。
