...
- 当前正式服务 `wheelleg-native-formal-1024.service` 已实际启动且已执行PPO更新；截至本条快照为第3轮、累计新增5836800步（动态状态以文件为准）。每轮新增2048000步，中点/末点共同CPU评估；从当前最佳恢复，差轮次不覆盖最佳，回滚不抹掉已经消耗的预算；停止后最佳只做一次新64例终评，不再据此调参。
- 唯一正式目录 `wheelleg_warp/results/formal_native_1024_20260921/`，入口 `train_native.py`。页面 http://127.0.0.1:8765/，`wheelleg-dashboard.service` 与 `wheelleg-training-render.service` 运行。训练24GiB、渲染3GiB内存上限且禁止对应进程组换页，环境上限1024，不再运行2048/4096。
...
- 报告与曲线：`formal_native_1024_20260921/REPORT.md`、`convergence.png`。末次退化、2048中断、原CPU/GPU全状态等价未全过等限制保留。当前仍在正式多轮优化，不能称最终最优已证明；最终成绩看status/selection/final_evaluation。
