...
当前状态：任务契约v2的160例公开开发基线与重复性校对已完成；terrain-v3检查点困难台阶三次32/32/33，原GPU检查点35/35/35，二者均未达到强准入。mixed两者均12/12，原场景存在个别结局翻转。同输入零残差GPU重复在首个50Hz策略步已出现微小状态与接触顺序差异，尚未证明放大链的唯一原因。无新训练、无权重晋升、720000留出未开；先定位动作/接触差异并冻结新开发场景，详见`wheelleg_warp/results/contract_v2_baseline_checked_20260923/REPORT.md`。姿态目标、实际扭矩硬限及纵向错相课程尚未改动。
