...
class Residual:
    ...
    def map(self, model, data, action):
        ...
        if self.mode == 'virtual6':
            ...
            if self.mode in ('mixed3_plus', 'mixed3_minus'):
                common_sign = 1 if self.mode == 'mixed3_plus' else -1
                virtual[:2] = (a[:2] * [force_scale, 1.])[:, None] * (
                    math.cos(math.pi / 6) * np.array([1., -1.]) + common_sign * .5)
