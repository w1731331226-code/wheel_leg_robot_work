...
BASELINE = Path(__file__).resolve().parent / 'results/preppo_2026-09-16/baseline'
OUT = Path(__file__).resolve().parent / 'results/fixes_2026-09-17/interface'


def run():
    rows = []
    OUT.mkdir(parents=True, exist_ok=True)
    manifest = json.loads((Path(__file__).resolve().parents[1] / 'BASELINE_MANIFEST.json').read_text())
    for name, expected in manifest['sha256'].items():
        assert hashlib.sha256((BASELINE / name).read_bytes()).hexdigest() == expected, name
    rows.append('original baseline: all frozen SHA256 hashes unchanged')
...
'''
    ...
    for speed in (-1, 1):
        files = []
        for label, source in (('old', BASELINE/'tools'), ('new', Path(__file__).parent)):
        ...
        assert all(np.isfinite(trace).all() for trace in files)
        differences.append(dict(speed=speed, max_abs_difference=float(np.max(abs(files[0]-files[1])))))
    (OUT/'baseline_difference.json').write_text(json.dumps(differences, indent=2)+'\n')
    rows.append('old/current drive-stop trajectories retained; model-change differences reported')
