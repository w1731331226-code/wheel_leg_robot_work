...
def run(method, seed, output, smoke, protocol='v1'):
    frozen = FROZEN if protocol == 'v1' else ROOT/'tools/results/yaw_precision_v2_2026-09-21'
    manifest = verify_sources(frozen)
    config_path = frozen/'training_config.json'
    config = json.loads(config_path.read_text())
    if method not in config['methods']:
        raise ValueError('Method is not registered in '+config['protocol'])
    ...
    if not smoke:
        gate = (ROOT/'tools/results/pretrain_yaw_2026-09-21/readiness.json'
                if protocol == 'v1' else frozen/'readiness.json')
        ...
        if not readiness.get('research_ready', False):
            raise RuntimeError('Research protocol readiness is not complete')
        for name, digest in readiness['source_sha256'].items():
            if hashlib.sha256((ROOT/name).read_bytes()).hexdigest() != digest:
                raise RuntimeError('Readiness evidence is stale: '+name)
    output.mkdir(parents=True, exist_ok=False)
    source_paths = [Path(__file__), ROOT/'tools/pretrain_yaw.py', config_path, frozen/'protocol_manifest.json']
    ...
    parameter_count = sum(value.numel() for value in model.policy.parameters())
    ...
    try:
        ...
        write(output/'completed.json',dict(method=method, protocol=config['protocol'], smoke=smoke,
              passed=True, policy_steps=steps, policy_parameter_count=parameter_count,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', choices=['M','M3','M1','N3+','N3-','B2-V','B2'], required=True)
    ...
    run(args.method, args.seed, args.output, args.smoke, args.protocol)
