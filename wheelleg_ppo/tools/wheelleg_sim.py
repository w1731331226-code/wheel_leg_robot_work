...
KD_WY = 0.02
KP_WZ = 0.05
YAW6_DAMPING = 2.0  # Nm/(rad/s)，名义航向角速度阻尼，实机需回标。
YAW6_LIMIT = 0.12  # 原质量/轮径归一差矩；7kg/50mm轮半径约1.65Nm/侧。
...
def control(m, d, st, residual=None):
    ...
    if st.jp == 'FLY':
        ...
        if st.lqr6 is not None:
            ...
            yaw_limit = YAW6_LIMIT * mass_scale * hw.WHEEL_RADIUS / 0.025
        # 六状态硬件模型需要更强的角速度阻尼来抑制单轮越障瞬态；位置增益保持不变。
        yaw_damping = YAW6_DAMPING if st.lqr6 is not None else 0.15
