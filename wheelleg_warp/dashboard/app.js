...
function update(s){
 ...
 if(s.validation){const v=s.validation;$('notice-body').textContent=`1024准入验证：${v.current??v.phase??'准备中'} · 最近完成 ${num(v.policy_steps)} / ${num(v.target)} 步。下方为采集预检的真实帧，正式启动后接入正式训练流。`;}
