...
if __name__=='__main__':
    ...
    p['development_cases']=p['development_cases'][:4];p['final_cases']=p['development_cases'];p['probe_evaluation_scope']='仅开发集4例，未使用正式最终保留集'
    ...
    with (output/'renderer.log').open('w') as log:
        ...
        try:
            ...
            write(output/'verified.json',dict(passed=True,updates=row['updates'],steps=p['steps_per_round'],restored_optimizer_and_normalization=True,probe_weights_promoted_to_formal=False,initialization_from_formal_bootstrap=True))
        finally:
