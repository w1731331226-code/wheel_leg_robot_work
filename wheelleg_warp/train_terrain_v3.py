...
def score(row):
    ...
    return (terrain['total']+legacy['total']-terrain['success_count']-legacy['success_count'],legacy['total']-legacy['success_count'],sum(not x['success'] for x in mixed),terrain['mean_yaw_score_deg']+legacy['mean_yaw_score_deg'])


def verify(output):
    protocol=json.loads((output/'protocol.json').read_text())
    for name,digest in protocol['source_sha256'].items():assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    return protocol
...
def initialize(output,source,candidate):
    ...
    protocol=json.loads((candidate/'protocol.json').read_text());initial=json.loads((candidate/'initial_evaluation.json').read_text());model=TimedPPO.load(str(checkpoint)+'.zip',device='cpu')
    paths=[Path(__file__),ROOT/'wheelleg_warp/terrain_eval.py',ROOT/'wheelleg_warp/native/terrain.py',ROOT/'wheelleg_warp/native/terrain_env.py',ROOT/'wheelleg_warp/native/models.py',ROOT/'wheelleg_warp/native/environment.py',ROOT/'wheelleg_warp/native/controller.py',ROOT/'wheelleg_warp/native/live.py',ROOT/'wheelleg_warp/dashboard/live_env.py']
    protocol.update(name='terrain-v3',status='ready',inherited_steps=model.num_timesteps,continuation_from='terrain-v2',bootstrap_summary=initial['terrain'],source_sha256={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in paths})
    initial.update(round=0,path=str(checkpoint),policy_steps=model.num_timesteps,summary=initial['terrain']);initial.pop('checkpoint',None)
    write(output/'protocol.json',protocol);write(output/'initial.json',initial);write(output/'selection.json',dict(best=initial,anchor=initial,stagnant_rounds=0,rounds=[]));write(output/'status.json',dict(status='ready',round=0,new_steps=0,initial_score=score(initial)))
...
def run_round(output,index):
    ...
    try:
        for additional in (protocol['steps_per_round']//2,protocol['steps_per_round']):
            ...
            record=dict(round=index,path=str(checkpoint),policy_steps=model.num_timesteps,round_steps=additional,train_seconds=train_seconds,terrain=summarize(terrain_rows,[x.terrain_seed for x in development]),legacy=summarize(legacy_rows,[x.terrain_seed for x in legacy_cases]),terrain_runs=terrain_rows,legacy_runs=legacy_rows,total_seconds=time.perf_counter()-started,updates=len(model.timings));record['summary']=record['terrain'];records.append(record);write(directory/(checkpoint.name+'.json'),record)
        chosen=min(records,key=score);verify(output);write(directory/'completed.json',dict(passed=True,round=index,stage=stage,records=records,selected=chosen,last=records[-1],train_seconds=train_seconds,total_seconds=time.perf_counter()-started))
...
def orchestrate(output):
    ...
    for index in range(1,protocol['max_rounds']+1):
        ...
        row=json.loads((output/f'round_{index:03d}/completed.json').read_text())['selected'];selection['rounds'].append(row)
        if score(row)<score(selection['best']):selection['best']=row
        if score(row)<score(selection['anchor']):selection['anchor']=row;selection['stagnant_rounds']=0
    ...
    best=selection['best'];model=TimedPPO.load(best['path']+'.zip',device='cpu');write(output/'status.json',dict(status='final_evaluation',round=index,new_steps=index*protocol['steps_per_round']))
