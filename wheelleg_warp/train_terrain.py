...
def score(terrain,legacy):
    return (terrain['total']+legacy['total']-terrain['success_count']-legacy['success_count'],
            legacy['total']-legacy['success_count'],terrain['mean_yaw_score_deg']+legacy['mean_yaw_score_deg'])
...
def initialize(output,source):
    ...
    initial=dict(round=0,path=str(output/'bootstrap/policy'),terrain=summarize(tr,[s.terrain_seed for s in development]),
    ...
    write(output/'status.json',dict(status='ready',round=0,new_steps=0,initial_score=score(initial['terrain'],initial['legacy'])))
...
def run_round(output,index):
    ...
    try:
        for extra in (p['steps_per_round']//2,p['steps_per_round']):
            ...
            record=dict(round=index,path=str(prefix),policy_steps=model.num_timesteps,round_steps=extra,train_seconds=train_seconds,
        ...
        chosen=min(records,key=lambda x:score(x['terrain'],x['legacy']));protocol(output)
...
def orchestrate(output):
    ...
    for index in range(1,p['max_rounds']+1):
        ...
        if new<old:selection['best']=row
        anchor=score(selection['anchor']['terrain'],selection['anchor']['legacy'])
        if new<anchor:selection['anchor']=row;selection['stagnant_rounds']=0
