...
def reduced_design(model, reference, a, b, order):
    """离散静态凝聚候选；必须检查被消去动态、频响及完整 15 状态闭环。"""
    if order not in (6, 8):
    ...
    at = transform @ full_a @ np.linalg.inv(transform)
    bt = transform @ full_b
    aff = at[order:, order:]
    fast = np.linalg.solve(np.eye(15 - order) - aff,
                           np.c_[at[order:, :order], bt[order:]])
    ar = at[:order, :order] + at[:order, order:] @ fast[:, :order]
    br = bt[:order] + at[:order, order:] @ fast[:, order:]
    # ponytail: 静态凝聚仅作低频候选；失真时保留动态/观测器，不靠提高增益补偿。
    ...
    frequencies = [0.2, 0.5, 1.0, 2.0, 5.0]
    errors = []
    ...
    omitted_radius = float(max(abs(np.linalg.eigvals(aff))))
    radius = float(max(abs(np.linalg.eigvals(a - b @ gain))))
    report = dict(order=order, vmc_condition=condition, frequencies_hz=frequencies,
    ...
    print('reduction ' + json.dumps(report), flush=True)
    return gain, report
...
if __name__ == '__main__':
    ...
    for height in args.heights:
        ...
        if args.reduction:
            gain, report = reduced_design(model, data, a, b, args.reduction)
