...
def control(m, d, st):
    dt = m.opt.timestep
    prior_peak_times = st.motor_peak_t.copy() if st.lqr6 is not None else None
    st.boot_t += dt
    ...
    for name, torque, speed in (('motor_alphaL', taL, qdaL), ('motor_betaL', tbL, qdbL),
        ...
        if st.hardware:
            torque, st.motor_peak_t[name] = hw.torque_limit(
                torque, speed, True, st.motor_peak_t.get(name, 0.0), dt)
        d.ctrl[aid] = torque
    ...
    for name, torque, speed in (('motor_wheelL', wheel_left, ws1),
        ...
        if st.hardware:
            torque, st.motor_peak_t[name] = hw.torque_limit(
                torque, speed, False, st.motor_peak_t.get(name, 0.0), dt)
        d.ctrl[aid] = torque


    if st.lqr6 is not None:
        st.lqr6.apply(m, d, st, prior_peak_times)
