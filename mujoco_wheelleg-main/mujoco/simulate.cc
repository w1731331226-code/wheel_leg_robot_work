...
// 驾驶模式 (M 键切换): ON 时 WASD 控制机器人; OFF 时恢复平台默认按键, 避免冲突
static bool g_drive_mode = false;
...
void UpdateWatch(mj::Simulate* sim, const mjModel* m, const mjData* d) {
  ...
  // Watch 功能已在 MuJoCo 3.12 移除 (MJDATA_POINTERS_PREAMBLE 宏不存在)
  (void)m; (void)d;
}
...
void UiEvent(mjuiState* state) {
  ...
  // ---- 按键调试: 任何按键都打印键码 (WHEELLEG_KEYDEBUG=1 开启) ----
  if (state->type==mjEVENT_KEY && std::getenv("WHEELLEG_KEYDEBUG")) {
    printf("[key] code=%d drive_mode=%d\n", state->key, (int)g_drive_mode);
  }

  // ---- 按键控制: 速度/转向由 PhysicsLoop 每步轮询键盘实现 (按住前进/松开停止) ----
  // 此处仅处理: ↑/↓ 腿长 (按一下 ±1cm), M 切换模式提示
  if (state->type==mjEVENT_KEY) {
    switch (state->key) {
      case mjKEY_UP:
        wheelleg::SetLegRef(wheelleg::GetLegRef() + 0.01f);
        printf("[cmd] 腿长指令 %+.2f m (机身升高)\n", wheelleg::GetLegRef());
        return;
      case mjKEY_DOWN:
        wheelleg::SetLegRef(wheelleg::GetLegRef() - 0.01f);
        printf("[cmd] 腿长指令 %+.2f m (机身降低)\n", wheelleg::GetLegRef());
        return;
      default:
        break;
    }
  ...
  if (state->type==mjEVENT_KEY && state->key!=0) {
    ...
    // ---- 轮腿机器人控制 ----
    // 速度/转向: 由 PhysicsLoop 每步轮询键盘实现 "按住前进/松开停止"
    //           (小键盘 8/5/4/6 + WASD, 不受 UI 控件/输入法拦截影响)
    // ↑/↓: 腿长 (按一下 ±1cm); 平台其他键保持不变
    case mjKEY_UP:
      wheelleg::SetLegRef(wheelleg::GetLegRef() + 0.01f);
      printf("[cmd] 腿长指令 %+.2f m (机身升高)\n", wheelleg::GetLegRef());
      break;
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    {
      ...
      if (!is_passive_) {
        ...
        if (ngeom < nusergeom) {
          printf("[simulate] scene geom buffer full: %d/%d\n", ngeom, this->scn.maxgeom);
        }
