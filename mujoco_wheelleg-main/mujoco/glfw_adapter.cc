...
bool GlfwAdapter::IsKeyDown(int key) const {
  // 键码与 GLFW 一致 (TranslateKeyCode 透传), 直接查询物理按键状态
  return Glfw().glfwGetKey(window_, key) == GLFW_PRESS;
}

void GlfwAdapter::PollEvents() {
  Glfw().glfwPollEvents();
  // 主线程轮询键盘 (glfwGetKey 仅限主线程), 结果供物理线程读取
  // 小键盘 8/5/4/6 无条件; WASD 同样支持
  int vel = 0, turn = 0;
  if (IsKeyDown(GLFW_KEY_KP_8) || IsKeyDown(GLFW_KEY_W)) vel = 1;
  if (IsKeyDown(GLFW_KEY_KP_5) || IsKeyDown(GLFW_KEY_S)) vel = -1;
  if (IsKeyDown(GLFW_KEY_KP_4) || IsKeyDown(GLFW_KEY_A)) turn = 1;
  if (IsKeyDown(GLFW_KEY_KP_6) || IsKeyDown(GLFW_KEY_D)) turn = -1;
  wheelleg::SetKeyHold(vel, turn);
  // 空格 / 7 / 小键盘7: 跳跃请求 (边沿: 按下置位, 控制器消费后清零)
  static bool sp_prev = false;
  const bool sp_now = IsKeyDown(GLFW_KEY_SPACE) || IsKeyDown(GLFW_KEY_7) || IsKeyDown(GLFW_KEY_KP_7);
  if (sp_now && !sp_prev) wheelleg::SetJumpReq(true);
  sp_prev = sp_now;
}
